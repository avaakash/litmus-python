...
def CreateEvents(eventsDetails, chaosDetails, kind, eventName, clients):
    ...
    try:
        ...
        return ValueError(f"Failed to create event with err: {exp}")
    return None
...
def GenerateEvents(eventsDetails, chaosDetails, kind, clients):
    ...
    if kind == "ChaosResult":
        ...
        err = CreateEvents(eventsDetails, chaosDetails,
                           kind, eventName, clients)
        if err is not None:
            return err
    elif kind == "ChaosEngine":
        eventName = eventsDetails.Reason + \
            chaosDetails.ExperimentName + str(chaosDetails.ChaosUID)
        event = client.V1Event
        try:
            event = clients.clientCoreV1.read_namespaced_event(
                name=eventName, namespace=chaosDetails.ChaosNamespace)
        except Exception as exp:
        ...
        try:
            clients.clientCoreV1.patch_namespaced_event(
                eventName, chaosDetails.ChaosNamespace, body=event)
        except Exception as exp:
            return exp
    return None
