...
def GetENV(experimentDetails):
    ...
    experimentDetails.ChaosDuration = maths.atoi(
        os.getenv("TOTAL_CHAOS_DURATION", "30"))
    ...
    experimentDetails.Timeout = maths.atoi(
        os.getenv("STATUS_CHECK_TIMEOUT", "180"))
    experimentDetails.TargetPods = os.getenv("TARGET_PODS", "")
    experimentDetails.PodsAffectedPerc = maths.atoi(
        os.getenv("PODS_AFFECTED_PERC", "0"))
...
def InitialiseChaosVariables(chaosDetails, experimentDetails):
    ...
    appDetails.AnnotationKey = os.getenv(
